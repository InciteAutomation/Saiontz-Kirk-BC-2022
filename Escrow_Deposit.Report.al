report 50144 Escrow_Deposit
{
    DefaultLayout = RDLC;
    RDLCLayout = 'ReportLayouts/Escrow_Deposit.rdl';
        ...
        dataitem("Posted Deposit Header";"Posted Deposit Header")
        {
            ...
            dataitem(PageHeader;"Integer")
            {
                ...
                dataitem("Posted Deposit Line";"Posted Deposit Line")
                {
                    ...
                    column(Deposit_Detail;"Deposit Detail")
                    {
                    }
                    ...
                    dataitem(CustApplication;"Integer")
                    {
                        ...
                        trigger OnAfterGetRecord()begin
                            if Number = 1 then AppliedCustLedgEntry.Find('-')
                            else
                        ...
                        trigger OnPreDataItem()begin
                            if not PrintApplications or ("Posted Deposit Line"."Account Type" <> "Posted Deposit Line"."Account Type"::Customer)then CurrReport.Break;
                    ...
                    dataitem(VendApplication;"Integer")
                    {
                        ...
                        trigger OnAfterGetRecord()begin
                            if Number = 1 then AppliedVendLedgEntry.Find('-')
                            else
                        ...
                        trigger OnPreDataItem()begin
                            if not PrintApplications or ("Posted Deposit Line"."Account Type" <> "Posted Deposit Line"."Account Type"::Vendor)then CurrReport.Break;
                            SetRange(Number, 1, AppliedVendLedgEntry.Count);
                            AppliedVendLedgEntry.SetCurrentKey("Vendor No.", "Posting Date");
                            TotalAmountApplied:=0;
                        end;
                    }
                    trigger OnAfterGetRecord()var TempAppliedCustLedgEntry: Record "Cust. Ledger Entry" temporary;
                    TempAppliedVendLedgEntry: Record "Vendor Ledger Entry" temporary;
                    begin
                        case "Account Type" of "Account Type"::"G/L Account": begin
                            if GLAccount.Get("Account No.")then AccountName:=GLAccount.Name
                            else
                                AccountName:=StrSubstNo(Text001, GLAccount.TableCaption);
                            if Description = AccountName then Description:='';
                        end;
                        "Account Type"::Customer: begin
                            if Customer.Get("Account No.")then AccountName:=Customer.Name
                            else
                                AccountName:=StrSubstNo(Text001, Customer.TableCaption);
                            if Description = AccountName then //FIXME JHIII --set to blank
                                Description:=Customer.Name;
                        end;
                        "Account Type"::Vendor: begin
                            if Vendor.Get("Account No.")then AccountName:=Vendor.Name
                            else
                                AccountName:=StrSubstNo(Text001, Vendor.TableCaption);
                            if Description = AccountName then Description:=Vendor.Name;
                        end;
                        "Account Type"::"Bank Account": begin
                            if BankAccount2.Get("Account No.")then AccountName:=BankAccount2.Name
                            else
                                AccountName:=StrSubstNo(Text001, BankAccount2.TableCaption);
                            if Description = AccountName then Description:='';
                        end;
                        end;
                        if PrintApplications then case "Account Type" of "Account Type"::Customer: begin
                                AppliedCustLedgEntry.DeleteAll;
                                FilterDepositCustLedgerEntry("Posted Deposit Line", CustLedgEntry);
                                if CustLedgEntry.FindSet then repeat EntryAppMgt.GetAppliedCustEntries(TempAppliedCustLedgEntry, CustLedgEntry, false);
                                        if TempAppliedCustLedgEntry.FindSet then repeat AppliedCustLedgEntry:=TempAppliedCustLedgEntry;
                            ...
                            "Account Type"::Vendor: begin
                                ...
                                if VendLedgEntry.FindSet then repeat EntryAppMgt.GetAppliedVendEntries(TempAppliedVendLedgEntry, VendLedgEntry, false);
                                        if TempAppliedVendLedgEntry.FindSet then repeat AppliedVendLedgEntry:=TempAppliedVendLedgEntry;
                    ...
                    trigger OnPostDataItem()begin
                        if not CurrReport.Preview then DepositPrinted.Run("Posted Deposit Header");
                    end;
                }
            }
            trigger OnAfterGetRecord()begin
                CurrReport.Language:=Language.GetLanguageIdOrDefault("Language Code");
                if not BankAccount.Get("Bank Account No.")then BankAccount.Name:=StrSubstNo(Text001, BankAccount.TableCaption);
                if "Currency Code" = '' then begin
                    if GLSetup."LCY Code" = '' then Currency.Description:=Text002
                    else if not Currency.Get(GLSetup."LCY Code")then Currency.Description:=StrSubstNo(Text001, FieldCaption("Currency Code"));
                end
                else if not Currency.Get("Currency Code")then Currency.Description:=StrSubstNo(Text001, FieldCaption("Currency Code"));
            end;
        }
    }
    requestpage
    ...
    local procedure GetCurrencyRecord(var Currency: Record Currency;
    CurrencyCode: Code[10])begin
    ...
    local procedure GetCurrencyCaptionCode(CurrencyCode: Code[10]): Text[80]begin
        if CurrencyCode = '' then exit('101,0,%1');
        GetCurrencyRecord(Currency, CurrencyCode);
        exit('101,4,' + Currency.Code);
    end;
    local procedure GetCurrencyCaptionDesc(CurrencyCode: Code[10]): Text[80]begin
        if CurrencyCode = '' then exit('101,1,%1');
        GetCurrencyRecord(Currency, CurrencyCode);
        exit('101,4,' + Currency.Description);
    end;
    procedure FilterDepositCustLedgerEntry(PostedDepositLine: Record "Posted Deposit Line";
    var DepositCustLedgerEntry: Record "Cust. Ledger Entry")var FromEntryNo: Integer;
    ToEntryNo: Integer;
    begin
        ...
        if PostedDepositLine.FindFirst then FromEntryNo:=PostedDepositLine."Entry No.";
    ...
    procedure FilterDepositVendLedgerEntry(PostedDepositLine: Record "Posted Deposit Line";
    var DepositVendLedgerEntry: Record "Vendor Ledger Entry")var FromEntryNo: Integer;
    ToEntryNo: Integer;
    begin
        PostedDepositLine.Reset;
        PostedDepositLine.SetCurrentKey("Deposit No.", "Line No.");
