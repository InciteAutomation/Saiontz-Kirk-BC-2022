codeunit 50150 "Escrow_Deposit-Post"
{
    ...
    trigger OnRun()var GLEntry: Record "G/L Entry";
    ...
    CurrLineNo: Integer;
    begin
        // Message('new post deposit ran');
        OnBeforeDepositPost(Rec);
        // Check deposit
        TestField(Rec."Posting Date");
        ...
        if "Total Deposit Lines" <> "Total Deposit Amount" then Error(Text000, FieldCaption("Total Deposit Amount"), FieldCaption("Total Deposit Lines"));
        ...
        TotalAmountLCY:=0;
        CurrLineNo:=0;
        Window.Open(StrSubstNo(Text001, "No.") + Text004 + Text002 + Text003);
        ...
        if GenJnlLine.Find('-')then repeat NextLineNo:=NextLineNo + 1;
                Window.Update(2, NextLineNo);
                InsertPostedDepositLine(Rec, GenJnlLine, NextLineNo);
                if GenJnlTemplate."Force Doc. Balance" then AddBalancingAccount(GenJnlLine, Rec)
                else
        ...
        GenJnlLine.SetRange("Journal Template Name", "Journal Template Name");
        GenJnlLine.SetRange("Journal Batch Name", "Journal Batch Name");
        if GenJnlLine.Find('-')then repeat CurrLineNo:=CurrLineNo + 1;
                ...
                case GenJnlLine."Account Type" of GenJnlLine."Account Type"::"G/L Account", GenJnlLine."Account Type"::"Bank Account": begin
                    GLEntry.FindLast;
                    PostedDepositLine."Entry No.":=GLEntry."Entry No.";
                    if GenJnlTemplate."Force Doc. Balance" and (GenJnlLine.Amount * GLEntry.Amount < 0)then PostedDepositLine."Entry No.":=PostedDepositLine."Entry No." - 1;
                end;
                ...
                if GenJnlTemplate."Force Doc. Balance" then begin
                    ...
                    if(GenJnlLine."Account Type" = GenJnlLine."Account Type"::"Bank Account") and (GenJnlLine.Amount * BankAccountLedgerEntry.Amount > 0)then PostedDepositLine."Entry No.":=PostedDepositLine."Entry No." - 1;
                end;
        ...
        if not GenJnlTemplate."Force Doc. Balance" then begin
            ...
            if PostedDepositLine.FindSet(true)then repeat PostedDepositLine."Bank Account Ledger Entry No.":=BankAccountLedgerEntry."Entry No.";
                    PostedDepositLine.Modify;
    ...
    var PostedDepositHeader: Record "Posted Deposit Header";
    ...
    local procedure AddBalancingAccount(var GenJnlLine: Record "Gen. Journal Line";
    DepositHeader: Record "Deposit Header")begin
    ...
    local procedure CopyBankComments(DepositHeader: Record "Deposit Header")var BankCommentLine: Record "Bank Comment Line";
    ...
    begin
        ...
        if BankCommentLine.FindSet then repeat BankCommentLine2:=BankCommentLine;
    ...
    local procedure DeleteBankComments(DepositHeader: Record "Deposit Header")var BankCommentLine: Record "Bank Comment Line";
    begin
        BankCommentLine.Reset;
        BankCommentLine.SetRange("Table Name", BankCommentLine."Table Name"::Deposit);
        BankCommentLine.SetRange("Bank Account No.", DepositHeader."Bank Account No.");
    ...
    local procedure InsertPostedDepositHeader(var DepositHeader: Record "Deposit Header")var RecordLinkManagement: Codeunit "Record Link Management";
    begin
    ...
    local procedure InsertPostedDepositLine(DepositHeader: Record "Deposit Header";
    GenJnlLine: Record "Gen. Journal Line";
    LineNo: Integer)begin
        with PostedDepositLine do begin
            "Deposit No.":=DepositHeader."No.";
            "Line No.":=LineNo;
            "Account Type":=GenJnlLine."Account Type".AsInteger();
            "Account No.":=GenJnlLine."Account No.";
            "Document Date":=GenJnlLine."Document Date";
            "Document Type":=GenJnlLine."Document Type".AsInteger();
            ...
            "Deposit Detail":=GenJnlLine."Deposit Detail";
            "Case Number":=GenJnlLine."Case Number";
            "Client Name":=GenJnlLine."Client Name";
            OnBeforePostedDepositLineInsert(PostedDepositLine, GenJnlLine);
            Insert;
        end;
    end;
    local procedure PostBalancingEntry(DepositHeader: Record "Deposit Header";
    TotalAmountLCY: Decimal)var GenJnlLine: Record "Gen. Journal Line";
    begin
        with GenJnlLine do begin
            Init;
    ...
    local procedure OnAfterCheckDeposit(DepositHeader: Record "Deposit Header")begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnAfterDepositPost(DepositHeader: Record "Deposit Header";
    var PostedDepositHeader: Record "Posted Deposit Header")begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnBeforeDepositPost(var DepositHeader: Record "Deposit Header")begin
    end;
    [IntegrationEvent(false, false)]
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforePostGenJnlLine(var GenJnlLine: Record "Gen. Journal Line";
    DepositHeader: Record "Deposit Header")begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnBeforePostedDepositHeaderInsert(var PostedDepositHeader: Record "Posted Deposit Header";
    DepositHeader: Record "Deposit Header")begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnBeforePostedDepositLineInsert(var PostedDepositLine: Record "Posted Deposit Line";
    GenJnlLine: Record "Gen. Journal Line")begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnBeforePostedDepositLineModify(var PostedDepositLine: Record "Posted Deposit Line";
    GenJnlLine: Record "Gen. Journal Line")begin
    end;
}
