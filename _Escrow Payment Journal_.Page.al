page 50155 "Escrow Payment Journal"
{
            ...
            repeater(Control1)
            {
                ...
                field("Incoming Document Entry No.";"Incoming Document Entry No.")
                {
                    ...
                    trigger OnAssistEdit()begin
                        if "Incoming Document Entry No." > 0 then HyperLink(GetIncomingDocumentURL);
                    end;
                }
                ...
                field("Job Queue Status";"Job Queue Status")
                {
                    ...
                    trigger OnDrillDown()var JobQueueEntry: Record "Job Queue Entry";
                    begin
        ...
        area(navigation)
        {
            ...
            group("&Payments")
            {
                ...
                action(SuggestVendorPayments)
                {
                    ...
                    trigger OnAction()var SuggestVendorPayments: Report "Suggest Vendor Payments";
                    begin
                ...
                action(SuggestEmployeePayments)
                {
                    ...
                    trigger OnAction()var SuggestEmployeePayments: Report "Suggest Employee Payments";
                    begin
                ...
                group("Electronic Payments")
                {
                    ...
                    action(ExportPaymentsToFile)
                    {
                        ...
                        trigger OnAction()var BankExportImportSetup: Record "Bank Export/Import Setup";
                        ...
                        begin
                            ...
                            begin
                                ...
                                if GenJnlLine.FindFirst then begin
                                    repeat GenJnlLine.DeletePaymentFileErrors;
                                        if GenJnlLine."Currency Code" <> BankAccount."Currency Code" then GenJnlLine.InsertPaymentFileError(NoExportDiffCurrencyErr);
                                        if((GenJnlLine."Account Type" = GenJnlLine."Account Type"::"Bank Account") or (GenJnlLine."Bal. Account Type" = GenJnlLine."Bal. Account Type"::"Bank Account")) and ((GenJnlLine."Bank Payment Type" <> GenJnlLine."Bank Payment Type"::"Electronic Payment") and (GenJnlLine."Bank Payment Type" <> GenJnlLine."Bank Payment Type"::"Electronic Payment-IAT"))then GenJnlLine.InsertPaymentFileError(StrSubstNo(WrongBankPaymentTypeErr, FieldCaption("Bank Payment Type"), "Bank Payment Type"::"Electronic Payment", "Bank Payment Type"::"Electronic Payment-IAT"));
                                        if not GenJournalBatch."Allow Payment Export" then PaymentExportGenJnlCheck.AddBatchEmptyError(GenJnlLine, GenJournalBatch.FieldCaption("Allow Payment Export"), '');
                                        if GenJnlLine.Amount < 0 then GenJnlLine.InsertPaymentFileError(NoExportNegativeErr);
                                        if GenJnlLine."Recipient Bank Account" = '' then GenJnlLine.InsertPaymentFileError(RecipientBankAccountEmptyErr)
                                        else if not UseForElecPaymentChecked(GenJnlLine)then GenJnlLine.InsertPaymentFileError(UseForElecPaymentCheckedErr);
                                    until GenJnlLine.Next = 0;
                                end;
                                if BankAccount."Last Remittance Advice No." = '' then InsertPaymentFileError(LastRemittanceErr);
                                if GenJnlLine.HasPaymentFileErrorsInBatch then begin
                                    Commit;
                                    Error(HasErrorsErr);
                                end;
                                if "Bank Payment Type" = "Bank Payment Type"::"Electronic Payment" then BankExportImportSetup.Get(BankAccount."Payment Export Format")
                                else if "Bank Payment Type" = "Bank Payment Type"::"Electronic Payment-IAT" then BankExportImportSetup.Get(BankAccount."EFT Export Code");
                                if GenJnlLine.FindFirst then begin
                                    repeat //ExportNewLines := BulkVendorRemitReporting.ProcessLine(GenJnlLine);
                                ...
                                if ExportNewLines then begin
                                    ...
                                    BulkVendorRemitReporting.RunWithRecord(GenJnlLine)end;
                            end;
                            Window.Close;
                        end;
                    }
                    action(VoidPayments)
                    {
                        ...
                        trigger OnAction()var BankAccount: Record "Bank Account";
                        begin
                            if "Account Type" = "Account Type"::"Bank Account" then BankAccount.Get("Account No.");
                            if "Bal. Account Type" = "Bal. Account Type"::"Bank Account" then BankAccount.Get("Bal. Account No.");
                            if(BankAccount."Export Format" = 0) or (BankAccount."Export Format" = BankAccount."Export Format"::Other)then begin
                                GenJnlLine.CopyFilters(Rec);
                                if not EntriesToVoid(GenJnlLine, true)then Error(NoEntriesToVoidErr);
                                if GenJnlLine.FindFirst then GenJnlLine.VoidPaymentFile;
                            end
                            else
                            begin
                                GenJnlLine.Reset;
                                ...
                                GenJnlLine.SetRange("Journal Template Name", "Journal Template Name");
                                GenJnlLine.SetRange("Journal Batch Name", "Journal Batch Name");
                                if not EntriesToVoid(GenJnlLine, false)then Error(NoEntriesToVoidErr);
                                ...
                                if "Account Type" = "Account Type"::"Bank Account" then VoidTransmitElecPayments.SetBankAccountNo("Account No.")
                                else if "Bal. Account Type" = "Bal. Account Type"::"Bank Account" then VoidTransmitElecPayments.SetBankAccountNo("Bal. Account No.");
                    ...
                    action(TransmitPayments)
                    {
                        ...
                        begin
                            ...
                            if(BankAccount."Export Format" = 0) or (BankAccount."Export Format" = BankAccount."Export Format"::Other)then begin
                                GenJnlLine.CopyFilters(Rec);
                                if GenJnlLine.FindFirst then GenJnlLine.TransmitPaymentFile;
                            end;
                        end;
                    }
                }
                action("Void Check")
                {
                    ...
                    trigger OnAction()var ConfirmManagement: Codeunit "Confirm Management";
                    begin
                        TestField("Bank Payment Type", "Bank Payment Type"::"Computer Check");
                        TestField("Check Printed", true);
                        if ConfirmManagement.GetResponseOrDefault(StrSubstNo(Text000, "Document No."), true)then CheckManagement.VoidCheck(Rec);
                    end;
                }
                action("Void &All Checks")
                {
                    ...
                    begin
                        if ConfirmManagement.GetResponseOrDefault(Text001, true)then begin
                            ...
                            if GenJnlLine.Find('-')then repeat GenJnlLine2:=GenJnlLine;
            ...
            action(Approvals)
            {
                ...
                trigger OnAction()var GenJournalLine: Record "Gen. Journal Line";
        ...
        area(processing)
        {
            group("F&unctions")
            {
                ...
                action(PositivePayExport)
                {
                    ...
                    trigger OnAction()var GenJnlBatch: Record "Gen. Journal Batch";
                ...
                action(GenerateEFT)
                {
                    ...
                    trigger OnAction()var GenJournalBatch: Record "Gen. Journal Batch";
            ...
            group("P&osting")
            {
                ...
                action(Preview)
                {
                    ...
                    trigger OnAction()var GenJnlPost: Codeunit "Gen. Jnl.-Post";
                    begin
            ...
            group(Workflow)
            {
                ...
                action(CreateApprovalWorkflow)
                {
                    ...
                    trigger OnAction()var TempApprovalWorkflowWizard: Record "Approval Workflow Wizard" temporary;
                    begin
            ...
            group(Approval)
            {
                ...
                action(Comment)
                {
                    ...
                    ApprovalsMgmt: Codeunit "Approvals Mgmt.";
                    begin
                        if OpenApprovalEntriesOnJnlLineExist then ApprovalsMgmt.GetApprovalComment(Rec)
                        else if OpenApprovalEntriesOnJnlBatchExist then if GenJournalBatch.Get("Journal Template Name", "Journal Batch Name")then ApprovalsMgmt.GetApprovalComment(GenJournalBatch);
                    end;
                }
            }
            group("Page")
    ...
    trigger OnAfterGetCurrRecord()var GenJournalBatch: Record "Gen. Journal Batch";
    ...
    trigger OnAfterGetRecord()begin
        ...
        RecipientBankAccountMandatory:=IsAllowPaymentExport and (("Bal. Account Type" = "Bal. Account Type"::Vendor) or ("Bal. Account Type" = "Bal. Account Type"::Customer));
        SetAMCAppearance;
    ...
    trigger OnNewRecord(BelowxRec: Boolean)begin
        ...
        if not VoidWarningDisplayed then begin
            ...
            if not GenJnlTemplate."Force Doc. Balance" then Message(CheckCannotVoidMsg);
    ...
    begin
        ...
        if ClientTypeManagement.GetCurrentClientType = CLIENTTYPE::ODataV4 then exit;
        BalAccName:='';
    ...
    var Text000: Label 'Void Check %1?';
    ...
    local procedure CheckForPmtJnlErrors()var BankAccount: Record "Bank Account";
    BankExportImportSetup: Record "Bank Export/Import Setup";
    begin
        if HasPmtFileErr then if("Bal. Account Type" = "Bal. Account Type"::"Bank Account") and BankAccount.Get("Bal. Account No.")then if BankExportImportSetup.Get(BankAccount."Payment Export Format")then if BankExportImportSetup."Check Export Codeunit" > 0 then CODEUNIT.Run(BankExportImportSetup."Check Export Codeunit", Rec);
    end;
    local procedure UpdateBalance()begin
        GenJnlManagement.CalcBalance(Rec, xRec, Balance, TotalBalance, ShowBalance, ShowTotalBalance);
        BalanceVisible:=ShowBalance;
        TotalBalanceVisible:=ShowTotalBalance;
        if ShowTotalBalance then NumberOfRecords:=Count();
    end;
    local procedure EnableApplyEntriesAction()begin
    ...
    local procedure CurrentJnlBatchNameOnAfterVali()begin
        CurrPage.SaveRecord;
        GenJnlManagement.SetName(CurrentJnlBatchName, Rec);
        SetControlAppearanceFromBatch;
        CurrPage.Update(false);
    end;
    local procedure GetCurrentlySelectedLines(var GenJournalLine: Record "Gen. Journal Line"): Boolean begin
        CurrPage.SetSelectionFilter(GenJournalLine);
        exit(GenJournalLine.FindSet);
    end;
    local procedure SetControlAppearanceFromBatch()var GenJournalBatch: Record "Gen. Journal Batch";
    ApprovalsMgmt: Codeunit "Approvals Mgmt.";
    WorkflowWebhookManagement: Codeunit "Workflow Webhook Management";
    CanRequestFlowApprovalForAllLines: Boolean;
    begin
        if("Journal Template Name" <> '') and ("Journal Batch Name" <> '')then GenJournalBatch.Get("Journal Template Name", "Journal Batch Name")
        else if not GenJournalBatch.Get(GetRangeMax("Journal Template Name"), CurrentJnlBatchName)then exit;
    ...
    local procedure CheckOpenApprovalEntries(BatchRecordId: RecordID)var ApprovalsMgmt: Codeunit "Approvals Mgmt.";
    begin
    ...
    begin
        OpenApprovalEntriesExistForCurrUser:=OpenApprovalEntriesExistForCurrUserBatch or ApprovalsMgmt.HasOpenApprovalEntriesForCurrentUser(RecordId);
    ...
    local procedure SetControlVisibility()var GLSetup: Record "General Ledger Setup";
    begin
        GLSetup.Get;
    ...
    local procedure SetAMCAppearance()var BankAccount: Record "Bank Account";
    GenJournalBatch: Record "Gen. Journal Batch";
    begin
        AMCFormat:=false;
        if GenJournalBatch.Get("Journal Template Name", CurrentJnlBatchName)then if BankAccount.Get(GenJournalBatch."Bal. Account No.")then if GenJournalBatch."Bal. Account Type" = GenJournalBatch."Bal. Account Type"::"Bank Account" then if(BankAccount."Export Format" = 0) or (BankAccount."Export Format" = BankAccount."Export Format"::Other)then AMCFormat:=true;
    end;
    local procedure EntriesToVoid(GenJnlLine3: Record "Gen. Journal Line";
    AMC: Boolean): Boolean begin
        ...
        if AMC then GenJnlLine3.SetRange("Exported to Payment File", true)
        else
        begin
    ...
    local procedure UseForElecPaymentChecked(GenJnlLine3: Record "Gen. Journal Line"): Boolean var CustomerBankAccount: Record "Customer Bank Account";
    VendorBankAccount: Record "Vendor Bank Account";
    begin
        if GenJnlLine3."Bal. Account Type" <> GenJnlLine3."Bal. Account Type"::"Bank Account" then if GenJnlLine3."Bal. Account Type" = GenJnlLine3."Bal. Account Type"::Vendor then begin
                ...
                if VendorBankAccount.FindFirst then exit(VendorBankAccount."Use for Electronic Payments")end
            else if GenJnlLine3."Bal. Account Type" = GenJnlLine3."Bal. Account Type"::Customer then begin
                    ...
                    if CustomerBankAccount.FindFirst then exit(CustomerBankAccount."Use for Electronic Payments");
                end
                else
                    exit(true)
        else if GenJnlLine3."Account Type" <> GenJnlLine3."Account Type"::"Bank Account" then if GenJnlLine3."Account Type" = GenJnlLine3."Account Type"::Vendor then begin
                    VendorBankAccount.SetRange("Vendor No.", GenJnlLine3."Account No.");
                    VendorBankAccount.SetRange(Code, GenJnlLine3."Recipient Bank Account");
                    if VendorBankAccount.FindFirst then exit(VendorBankAccount."Use for Electronic Payments");
                end
                else if GenJnlLine3."Account Type" = GenJnlLine3."Account Type"::Customer then begin
    ...
    local procedure SetJobQueueVisibility()begin
        JobQueueVisible:="Job Queue Status" = "Job Queue Status"::"Scheduled for Posting";
        JobQueuesUsed:=GeneralLedgerSetup.JobQueueActive;
    end;
    [IntegrationEvent(false, false)]
    local procedure OnAfterValidateShortcutDimCode(var GenJournalLine: Record "Gen. Journal Line";
